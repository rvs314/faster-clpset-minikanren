;; Modified by William E. Byrd, July 16, 2025, to use sets rather than
;; lists for take/run results, and to use Chez Rather than Racket.
...
(define (take-allo $ s/c-set)
  (fresh ($1)
    ...
    (conde
      ...
      [(fresh (a d-s/c-set $d)
         (== `(,a . ,$d) $1)
         (== (set-cons a d-s/c-set) s/c-set)
         (take-allo $d d-s/c-set))])))
  ...
  (conde
    ...
    [(=/= '() n)
     (fresh ($1)
       ...
       (conde
         [(== '() $1) (== (set) s/c-set)]
         [(fresh (n-1 d-s/c-set a d)
            (== `(,a . ,d) $1)
            (== `(,n-1) n)
            (== (set-cons a d-s/c-set) s/c-set)
            (take-no n-1 d d-s/c-set))]))]))

#|
;; Original, list-based definitions of `take-allo` and `take-no`
